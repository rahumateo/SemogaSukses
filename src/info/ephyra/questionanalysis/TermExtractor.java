...
public class TermExtractor {
        ...
        for (int neId = 0; neId < nes.length; neId++) {
            for (String ne : nes[neId]) {
                if (term.equals(ne)) {
                    ...
                    if (neTypesSet.add(neType)) // there may be multiple taggers (IDs) for one type
                    {
                        neTypes.add(neType);
                    }
                    break;
    ...
    public static String[][] getNes(String sentence, String context) {
        ...
        for (int i = 0; i < nes.length; i++) {
            if (sentenceNes[i].length == 0) {
                nes[i] = contextNes[i];
            } else if (contextNes[i].length == 0) {
                nes[i] = sentenceNes[i];
            } else {
                ArrayList<String> nesL = new ArrayList<String>();
                for (String ne : sentenceNes[i]) {
                    nesL.add(ne);
                }
                for (String ne : contextNes[i]) {
                    nesL.add(ne);
                }
    ...
    public static Term[] getTerms(String sentence, String[][] nes,
        ...
        for (int length = MAX_TERM_LENGTH; length > 1; length--) {
            for (int id = 0; id < tokens.length - length + 1; id++) {
                ...
                if (!termSet.add(StringUtils.normalize(text))) {
                    continue;
                }
                // phrase does not contain keywords?
                if (KeywordExtractor.getKeywords(text).length == 0) {
                    continue;
                }
                ...
                for (Dictionary dict : dicts) {
                    ...
                    if (!text.equals(TruncationFilter.truncate(text))) {
                        continue;
                    }
        ...
        for (int id = 0; id < tokens.length; id++) {
            ...
            if (!termSet.add(StringUtils.normalize(tokens[id]))) {
                continue;
            }
            // token does not contain keywords?
            if (KeywordExtractor.getKeywords(tokens[id]).length == 0) {
                continue;
            }
